...
const SmallCard = ({ icon, text, value, unit, size }) => {
  if (!icon) {
    return (
      <View className="h-28 w-40 bg-white/20 items-center p-3 mt-5 border-4 border-t-0 border-b-8 border-black/20 rounded-3xl  justify-around"></View>
    );
  }
  return (
    <View className="h-28 w-40 bg-white/20 items-center p-3 mt-5 border-4 border-t-0 border-b-8 border-black/20 rounded-3xl  justify-around">
      ...
      <Text className="font-poppins color-white justify-self-end">
        {value} {unit}
      </Text>
    </View>
  );
